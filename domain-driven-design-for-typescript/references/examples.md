...
export const createMoney = (input: z.input<typeof Money>): Money => Money.parse(input);
...
export const calculateOrderTotal = (order: Order): Money => {
  ...
  return order.lines.reduce((sum, line) => addMoney(sum, calculateLineTotal(line)), {
    amount: 0,
...
// DO NOT export fixtures files
export * from "./athlete.fixtures.ts";
```
...
export const makeOrderLine = (overrides?: Partial<OrderLine>): OrderLine => ({
